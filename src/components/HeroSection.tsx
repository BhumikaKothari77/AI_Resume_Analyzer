import { Zap, Shield, Target, Sparkles } from 'lucide-react';

export default function HeroSection() {
  return (
    <section className="text-center pt-14 pb-10 sm:pt-18 sm:pb-14 px-4">
      {/* Top Tag */}
      <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-pink-50/80 border border-pink-200/60 text-xs font-semibold text-pink-700 mb-6 shadow-xs animate-fade-in">
        <Sparkles className="w-3.5 h-3.5 text-pink-500" />
        <span>Next-Gen AI Resume & ATS Diagnostic</span>
      </div>

      <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 tracking-tight leading-tight max-w-3xl mx-auto">
        Know exactly what's holding
        <br className="hidden sm:block" />
        <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-600 via-rose-500 to-fuchsia-600">
          {' '}your resume back.
        </span>
      </h1>

      <p className="mt-5 text-base sm:text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
        Upload your resume and get an ATS-focused breakdown of formatting,
        keywords, section completeness, and bullet point rewrites.
      </p>

      {/* Trust indicators */}
      <div className="mt-8 flex flex-wrap items-center justify-center gap-4 sm:gap-6 text-sm text-gray-600">
        <div className="flex items-center gap-1.5 bg-white/60 px-3 py-1.5 rounded-xl border border-pink-100/60 shadow-xs">
          <Shield className="w-4 h-4 text-pink-500" />
          <span>ATS-focused analysis</span>
        </div>
        <div className="hidden sm:block w-1 h-1 rounded-full bg-pink-300" />
        <div className="flex items-center gap-1.5 bg-white/60 px-3 py-1.5 rounded-xl border border-pink-100/60 shadow-xs">
          <Target className="w-4 h-4 text-rose-500" />
          <span>Recruiter-level insights</span>
        </div>
        <div className="hidden sm:block w-1 h-1 rounded-full bg-pink-300" />
        <div className="flex items-center gap-1.5 bg-white/60 px-3 py-1.5 rounded-xl border border-pink-100/60 shadow-xs">
          <Zap className="w-4 h-4 text-fuchsia-500" />
          <span>Real-time instant results</span>
        </div>
      </div>
    </section>
  );
}
